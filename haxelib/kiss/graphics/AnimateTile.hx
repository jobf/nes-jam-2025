...
class AnimateMosaic
{
	...
	function new(animations:Map<String, MosaicConfig>, mosaic:Mosaic)
	{
		this.animations = animations;
		this.mosaic = mosaic;
		var anims = [for (k in animations.keys()) k];
		play_animation(anims[0]);
	}

	function play_animation(name:String):Void
	{
		if (now_playing_name != name && animations.exists(name))
		{
			now_playing_name = name;
			now_playing = animations[name];
			frames_remaining = now_playing.frame_rate;
			position = 0;
			change_frame();
			is_animation_ended = false;
	...
	function change_frame()
	{
		...
		mosaic.arrange(now_playing.frames[position]);
		...
		// 	change_tile(c, r, tiles[n], isFlipped, paletteIndex);
